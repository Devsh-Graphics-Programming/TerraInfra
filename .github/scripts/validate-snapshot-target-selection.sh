...
snapshot_rocket_matrix="$(read_output matrix "${snapshot_rocket}")"
jq -e '.include | length == 1 and .[0].target_key == "rocket" and .[0].volume_name == "prod-rocket-01-data"' <<< "${snapshot_rocket_matrix}" >/dev/null
...
restore_rocket_matrix="$(read_output matrix "${restore_rocket}")"
jq -e '.include | length == 1 and .[0].target_key == "rocket" and .[0].instance_type == "DEV1-M"' <<< "${restore_rocket_matrix}" >/dev/null
