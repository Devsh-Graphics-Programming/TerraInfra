...
- `jenkins`: restored Jenkins home includes controller config, master key, plugins, and the managed smoke job; the restored controller starts locally, `/login` responds, and `/prometheus/` is present but requires authentication.
