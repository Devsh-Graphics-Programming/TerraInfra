        ...
        withEnv([
          ...
          stage('Render scenes') {
            writeFile file: 'run-scenes.ps1', text: [
              ...
              '      $summary = $serializer.DeserializeObject([System.IO.File]::ReadAllText($summaryPath))',
              ...
              '      $runtimeImage = @{ "identifier" = "runtime"; "title" = "Runtime"; "status" = "error"; "status_color" = "red"; "details" = ("EX40 exited with code {0} before writing summary.json." -f $exitCode); "filename" = $display }',
              '      $runtimeScene = @{ "array" = @($runtimeImage); "compare" = $null; "details" = ("Command: {0}" -f $line); "display_name" = $display; "index" = $sceneNumber; "scene_name" = ("{0:D2}_{1}" -f $sceneNumber, $display); "scene_path" = $line; "sensor" = 0; "status" = "failed"; "status_color" = "red" }',
              ...
              '  }',
              '  if (-not $firstSummary) {',
              '    $firstSummary = @{ "buildConfig" = ""; "compare" = @{}; "datetime" = ""; "failure_count" = 0; "num_of_tests" = 0; "pass_status" = "failed"; "results" = @() }',
              '  }',
              '  $mergedResults = [object[]]$sceneResults.ToArray()',
              '  Write-Host ("Merging {0} isolated scene results." -f $mergedResults.Count)',
              '  $firstSummary["results"] = $mergedResults',
              '  $firstSummary["num_of_tests"] = $mergedResults.Count',
              ...
              '  Write-Host ("Writing merged isolated summary.json. tests={0}, failures={1}." -f $firstSummary["num_of_tests"], $firstSummary["failure_count"])',
              '  [System.IO.File]::WriteAllText($summaryPath, $serializer.Serialize($firstSummary), $utf8NoBom)',
