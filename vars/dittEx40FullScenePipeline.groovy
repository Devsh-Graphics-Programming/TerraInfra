        ...
        withEnv([
          ...
          stage('Render scenes') {
            writeFile file: 'run-scenes.ps1', text: [
              ...
              '    $sceneTimeoutSeconds = 300',
              '    $stdout = Join-Path $env:WORKSPACE ("ex40-" + [guid]::NewGuid().ToString("N") + ".stdout.log")',
              '    $stderr = Join-Path $env:WORKSPACE ("ex40-" + [guid]::NewGuid().ToString("N") + ".stderr.log")',
              '    $process = Start-Process -FilePath $package.exe -ArgumentList $runArgs -WorkingDirectory $package.bin -PassThru -RedirectStandardOutput $stdout -RedirectStandardError $stderr',
              '    try {',
              '      Wait-Process -Id $process.Id -Timeout $sceneTimeoutSeconds -ErrorAction Stop',
              '      $process.Refresh()',
              '      $runExitCode = $process.ExitCode',
              '    } catch {',
              '      if (-not $process.HasExited) {',
              '        Write-Warning ("EX40 scene process exceeded {0} seconds; terminating it." -f $sceneTimeoutSeconds)',
              ...
              '        Get-Content -LiteralPath $outputFile | Tee-Object -FilePath $log -Append | ForEach-Object { Write-Host $_ }',
